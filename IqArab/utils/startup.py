...
LOGS = logging.getLogger("تليثون اڤتار \n ")
...
async def startupmessage():
    try:
        if BOTLOG:
            Config.CATUBLOGO = await iqthon.tgbot.send_file(BOTLOG_CHATID, "https://telegra.ph/file/88b355899c7f69172dacc.jpg", caption="🝳 ⦙ تـمّ  اعـادة تشـغيل\n تليثـون اڤتار ✓  :  [ 8.2 ] .\n\n🝳 ⦙ للحصول على اوامر السورس\n أرسـل : (  `.اوامري`  ) \n\n🝳 ⦙ لمـعرفة كيفية تغير بعض كلايش\n او صور السـورس  أرسـل  :\n (  `.مساعده`  )\n\n🝳 ⦙ القناة الرسمية تليثون اڤتار : @sourceav\n🝳 ⦙ فارات سورس تليثون  :@Help_Tele_Av \n🝳 ⦙ كلايش تليثون :  @avTar_So\n 🝳 ⦙التحديثات والاضافات :  @Help_Tele_Av\n",                buttons=[(Button.url("مطور تليثون الرسمي", "https://t.me/Z_X_Z_B"),)],            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        _, groupid = await create_supergroup(            "تخزين تليثون اڤتار العام", iqthon, Config.TG_BOT_USERNAME, descript  ,  iqphoto1 )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        ...
        _, groupid = await create_supergroup(            "تخزين تليثون اڤتار الخاص", iqthon, Config.TG_BOT_USERNAME, descript    , iqphoto2  )
